...
def train_from_config(config_path,mode=None):
    ...
    train_ds=CachedMultimodalDataset(cache,"train",tcfg.get("modality_dropout",0.)); val_ds=CachedMultimodalDataset(cache,"val",0); shapes,target_shape=infer_shapes(train_ds)
    ...
    opt=torch.optim.AdamW(model.parameters(),lr=float(tcfg.get("lr",3e-4)),weight_decay=float(tcfg.get("weight_decay",1e-4))); outdir=ensure_dir(Path(cfg.get("output_dir","runs"))/dcfg["name"]/(mode or mcfg.get("mode","contrastive_topk"))); best=float("inf"); history=[]
    for epoch in range(1,int(tcfg.get("epochs",30))+1):
        ...
        if metrics["loss"]<best: best=metrics["loss"]; torch.save({"model":model.state_dict(),"shapes":shapes,"num_outputs":num_outputs,"task":task,"config":cfg},outdir/"best.pt")
